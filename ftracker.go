...
func RunningSpentCalories(action int, weight, duration float64) float64 {
    // ваш код здесь
    avgSpeedInKm := meanSpeed(action, duration)
    return ((runningCaloriesMeanSpeedMultiplier * avgSpeedInKm * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
    avgSpeedInM := meanSpeed(action, duration) * kmhInMsec
    return ((walkingCaloriesWeightMultiplier * weight + (math.Pow(avgSpeedInM, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
    // ваш код здесь
    avgSpeedInKm := swimmingMeanSpeed(lengthPool, countPool, duration)
    return (avgSpeedInKm + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
}
